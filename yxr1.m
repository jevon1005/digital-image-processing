...
%为每个滤波器设置合适的值
D0_ideal = 50;        % 理想高通：较小值控制振铃
D0_gaussian = 50;     % 高斯高通：较大值补偿平滑衰减
D0_butterworth = 30;  % 巴特沃斯：折中值
n = 2; % 巴特沃斯滤波器阶数
...
for i = 1:M
    for j = 1:N
        ...
        % 高斯高通滤波
        H_gaussian_hp(i,j) = 1 - exp(-d^2/(2*D0_gaussian^2));
        % 巴特沃斯高通滤波
        H_butterworth_hp(i,j) = 1/(1 + (D0_butterworth/d)^(2*n));
    end
end
...
subplot(2,2,2); imshow(I_ideal_hp, []); title(['理想高通滤波 D0=', num2str(D0_ideal)]);
subplot(2,2,3); imshow(I_gaussian_hp, []); title(['高斯高通滤波 D0=', num2str(D0_gaussian)]);
subplot(2,2,4); imshow(I_butterworth_hp, []); title(['巴特沃斯高通滤波 D0=', num2str(D0_butterworth), ' n=', num2str(n)]);